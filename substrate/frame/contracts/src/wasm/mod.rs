...
#[cfg(test)]
pub use {crate::wasm::runtime::ReturnErrorCode, runtime::STABLE_API_COUNT, tests::MockExt};

#[cfg(feature = "runtime-benchmarks")]
pub use crate::wasm::runtime::{BenchEnv, ReturnData, TrapReason};

pub use crate::wasm::runtime::{Environment, Runtime, RuntimeCosts};
...
pub struct PreparedCall<'a, E: Ext + 'a> {
	instance: polkavm::Instance<Runtime<'a, E>>,
	runtime: Runtime<'a, E>,
	entry_point: &'a ExportedFunction,
}

impl<'a, E: Ext> PreparedCall<'a, E> {
	pub fn call(mut self) -> ExecResult {
		let result = self.instance.call_typed(&mut self.runtime, self.entry_point.identifier(), ());
		let gas_left_engine = self.instance.gas_remaining().expect("Gas metering is enabled; qed");
		let _ = self.runtime.ext().gas_meter_mut().sync_from_executor(gas_left_engine.get());
		self.runtime.to_execution_result(result)
	}
}
...
impl<T: Config> WasmBlob<T> {
	...
	pub fn bench_prepare_call<E: Ext<T = T>>(
		...
		self.prepare_call(Runtime::new(ext, input_data), &ExportedFunction::Call)
	}

	fn prepare_call<'a, E: Ext<T = T>>(
		self,
		...
		let module = polkavm::Module::new(&engine, &module_config, code).map_err(|err| {
			log::debug!(target: LOG_TARGET, "failed to create polkavm module: {err:?}");
			Error::<T>::CodeRejected
		})?;

		let linker = polkavm::Linker::<Runtime<'a, E>>::new(&engine);
		let instance_pre = linker.instantiate_pre(&module).map_err(|err| {
			log::debug!(target: LOG_TARGET, "failed to link polkavm module: {err:?}");
			Error::<T>::CodeRejected
		})?;

		let instance = instance_pre.instantiate().map_err(|err| {
			log::debug!(target: LOG_TARGET, "failed to instantiate polkavm module: {err:?}");
			Error::<T>::CodeRejected
		})?;

		let gas_limit_engine = runtime
			.ext()
			.gas_meter_mut()
			.gas_left()
			.ref_time()
		...
		state_args.set_gas(polkavm::Gas::from_u64(gas_limit_engine).ok_or_else(|| "Gas to high")?);
		instance
		...
		Ok(PreparedCall { instance, runtime, entry_point })
	}
}
